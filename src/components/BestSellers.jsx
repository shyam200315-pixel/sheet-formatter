    ...
    <div className="w-full pb-16">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-normal tracking-tight text-[#202124] mb-1">
              Product & Store Analytics
            </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="google-card p-6 bg-gradient-to-br from-blue-50 to-white border border-blue-100 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-6 relative z-10">
            ...
            <h2 className="text-lg font-medium text-gray-800">Overall Best Selling Item</h2>
          </div>
          <div className="relative z-10">
            <div className="text-2xl md:text-3xl font-semibold text-gray-900 mb-2 truncate" title={insights.overallTopItem.name}>
              {insights.overallTopItem.name}
            </div>
            <div className="text-sm font-medium text-blue-600 bg-blue-50 inline-block px-3 py-1 rounded-full border border-blue-100">
              {insights.overallTopItem.qty.toLocaleString()} Units Sold
            </div>
          </div>
        </div>

        <div className="google-card p-6 bg-gradient-to-br from-purple-50 to-white border border-purple-100 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-10">
          ...
          <div className="flex items-center gap-3 mb-6 relative z-10">
            ...
            <h2 className="text-lg font-medium text-gray-800">Overall Best Category</h2>
          </div>
          <div className="relative z-10">
            <div className="text-2xl md:text-3xl font-semibold text-gray-900 mb-2 truncate" title={insights.overallTopCategory.name}>
              {insights.overallTopCategory.name}
            </div>
            <div className="text-sm font-medium text-purple-600 bg-purple-50 inline-block px-3 py-1 rounded-full border border-purple-100">
        ...
        <div className="p-6">
          {viewMode === "category" && (
            <div>
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <div>
                  <h2 className="text-xl font-medium text-gray-800">Analyze a Category</h2>
                  <p className="text-sm text-gray-500">See exactly how many units each store sold of a specific category.</p>
                </div>
                <div className="w-full md:w-72 shrink-0">
                  <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Select Category</label>
                  <select
                    ...
                    className="w-full bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 shadow-sm"
                  >
              ...
              {selectedCategory !== 'All' ? (
                <div>
                  <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-4 mb-6 flex justify-between items-center">
                    <span className="font-medium text-indigo-900">Total {selectedCategory} Sold:</span>
                        ...
                        {insights.storeBreakdown.map((item, index) => {
                          ...
                          return (
                            <React.Fragment key={item.store}>
                              ...
                              >
                                ...
                                <td className="px-6 py-4 font-medium text-gray-900">
                                  <div className="flex items-center gap-2">
                                    <Store size={16} className="text-gray-400 group-hover:text-indigo-500 transition-colors" />
                                    {item.store}
                                    {isExpanded ? (
                              ...
                              {isExpanded && (
                                <tr>
                                  <td colSpan="3" className="px-6 py-4 bg-gray-50/50 border-b border-gray-100">
                                    <div className="pl-14 pr-6">
                                      <h4 className="text-xs font-bold text-gray-400 uppercase mb-3 tracking-wider flex items-center gap-1">
                                        ...
                                        <ul className="divide-y divide-gray-100">
                                          {Object.entries(itemsMap)
                                            ...
                                            .map(([itemName, itemQty]) => (
                                              <li key={itemName} className="flex justify-between items-center py-2 px-4 hover:bg-gray-50 transition-colors">
                                                <span className="text-sm text-gray-600">{itemName}</span>
                                                <span className="font-medium text-sm text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">
                                                  {itemQty}
                                                </span>
                                              </li>
                                          ))}
                ...
                <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200 border-dashed">
                  ...
                  <p className="text-gray-600 font-medium">Select a category from the dropdown</p>
                  <p className="text-gray-400 text-sm mt-1">to view a detailed breakdown of sales across all stores.</p>
                </div>
              )}
            </div>
          )}
          ...
          {viewMode === "store" && (
            <div>
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                <div>
                  <h2 className="text-xl font-medium text-gray-800">Analyze a Store</h2>
                  <p className="text-sm text-gray-500">See all categories sold by a specific store. Click on a category to see specific items.</p>
                </div>
                <div className="w-full md:w-80 shrink-0">
                  <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Select Store</label>
                  <select
                    value={selectedStore}
                    onChange={handleStoreSelect}
                    className="w-full bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-emerald-500 focus:border-emerald-500 block p-2.5 shadow-sm"
                  >
              ...
              {selectedStore !== 'All' ? (
                          ...
                          return (
                            <React.Fragment key={item.category}>
                              ...
                              >
                                ...
                                <td className="px-6 py-4 font-medium text-gray-900">
                                  <div className="flex items-center gap-2">
                                    <LayoutList size={16} className="text-gray-400 group-hover:text-emerald-500 transition-colors" />
                                    {item.category}
                                    {isExpanded ? (
                                      <ChevronUp size={16} className="text-gray-400" />
                                    ) : (
                                        ...
                                        <ul className="divide-y divide-gray-100">
                                          {Object.entries(itemsMap)
                                            ...
                                            .map(([itemName, itemQty]) => (
                                              <li key={itemName} className="flex justify-between items-center py-2 px-4 hover:bg-gray-50 transition-colors">
                                                <span className="text-sm text-gray-600">{itemName}</span>
                                                <span className="font-medium text-sm text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">
                                                  {itemQty}
                                                </span>
                                              </li>
                                          ))}
                ...
                <div className="text-center py-12 bg-gray-50 rounded-xl border border-gray-200 border-dashed">
                  ...
                  <p className="text-gray-600 font-medium">Select a store from the dropdown</p>
                  <p className="text-gray-400 text-sm mt-1">to view a detailed breakdown of all categories sold by them.</p>
                </div>
              )}
            </div>
          )}
